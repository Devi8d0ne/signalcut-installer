...
SignalCut 0.2.6 strengthens **production recovery and publish safety**. YouTube publishing is idempotent, reconciles uploads after uncertain network failures, repairs thumbnail and playlist state, and records Shorts classification. The pipeline saves checkpoints, applies connection cooldowns without blocking healthy queued work, and shows live Codex activity. Director responses stream into the workspace with improved scroll following, while optional milestone voice notices keep hands-off runs observable.

SignalCut 0.2.5 expands the **local connector and resilience layer**. Settings includes validated presets for Openverse, Wikimedia Commons, NASA, Internet Archive, Crossref, Semantic Scholar, and Pixabay plus owner-defined HTTPS JSON connectors; credentials stay in the private runtime and local-network targets are rejected. Research files are automatically indexed into each project's source ledger. Codex and ElevenLabs stop new work at 20% remaining unless separately overridden by the owner. Project routing confirms saves, Publish review retries generated-package loading without clearing existing values, and generated thumbnails are previewed before upload.
